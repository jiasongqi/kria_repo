...
- 产物路径：`mydocs/codemap/YYYY-MM-DD_hh-mm_项目总图.md`
...
- `SWITCH / 切换 <project>` → 切换 `active_project`，自动加载 codemap
...
- 输出：
  - `feature`：`mydocs/codemap/YYYY-MM-DD_hh-mm_功能.md`
  - `project`：`mydocs/codemap/YYYY-MM-DD_hh-mm_项目总图.md`
- 要点：
...
- 输出：`mydocs/context/YYYY-MM-DD_hh-mm__context_bundle.md`
- 输出级别：
...
- 执行动作：
  ...
  - 形成首版研究结论与下一步动作
- 输出：`mydocs/specs/YYYY-MM-DD_hh-mm_<任务名>.md`
...
- 输出：
  - `human`：`mydocs/archive/YYYY-MM-DD_hh-mm__human.md`（汇报视角）
  - `llm`：`mydocs/archive/YYYY-MM-DD_hh-mm__llm.md`（后续开发参考视角）
